...
public class PostController {
	...
	public PostDto insert(String title, String author) {
		...
		//dao를 이용해서 dto 에 저장된 정보를 DB 에 저장하기
		dao.insert(dto);
		//현재 추가된 정보를 리턴하기 
		return dto;
	}
	*/
	 ...
	 * 
	 * spring 프레임웤에서는 @RequestBody 라는 어노테이션을 붙여주면 된다. 
	 * 
	 * 그러면 json 문자열의 key와 dto의 필드명이 일치하면 데티어가 추출되어서 필드에 저장된다.
	 * 
	 */
	
	//위의 요청 파라미터 추출을 좀 더 편한 방법으로 하면 아래와 같다.
	@PostMapping("/posts")
	public PostDto insert(@RequestBody PostDto dto) {//title과 author 가 추출되어서 PostDto 객체에 담긴채로 전달된다.
